...
- `/sudoku` 스도쿠: 일반 / X(대각선), 초급·중급·고급(처음 주어진 칸 38~40 / 30~34 / 24~28). 게임 상태(퍼즐·입력·힌트·메모·경과 시간)는 URL 쿼리 `?m=&d=&s=`에만 저장된다. 공유 버튼은 같은 문제를 빈 판으로 다시 푸는 링크를 복사하고, 완성 뒤에는 걸린 시간·힌트 횟수도 함께 담는다
- `/wordle` 워들: 한국 시간 자정마다 바뀌는 단어를 기본 자모 7개로 풀어 8번 안에 맞힌다. 오늘 진행과 통계는 브라우저 localStorage(`hardy:wordle`)에 저장된다. 결과 공유는 색 칸 격자와 접속 주소를 복사한다
...
- `src/lib/common`, `src/components/common`: 게임 공용 비트 코덱·localStorage 접근·시간 표시·타이머·모달·폭죽·클립보드 복사
- `src/lib/sudoku`, `src/components/sudoku`: 스도쿠 규칙·풀이기·생성기·URL 코덱·공유 문구·리듀서와 화면
